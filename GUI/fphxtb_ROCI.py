...
TESTBENCH_RESET       = 0x04 # Reset for both FEM and ROC = 0x`C4.  For just ROC = 0x04.  For just FEM = 0x44
...
def create_packet(dest,data,wedgeaddr=None,femaddr=None):
    ...
    if wedgeaddr is not None: length += 1 # one more byte for optional wedgeaddr
    if femaddr is not None: length += 1   # one more byte for optional femaddr
