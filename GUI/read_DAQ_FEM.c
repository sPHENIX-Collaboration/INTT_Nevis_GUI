	...
	try
	{
		...
		while ( take_data )	{
			...
			for (index = 0; index<1000000; index++) {
				if ( roc )
				{
					if ((data[index]&0xFFFFFFFF)!=0 || reading_data) 
					{
						evtcnt++;
							
							//printf("Data = %#x\n", data[index]);

							// Check for header and trailer words in data and only extract data words for writing to file:
							if ( ((data[index] & 0xFFFF) == 0xFFFF) && trailer_found){
								header_found = true;
								trailer_found = false;
								word_counter = 0;
							}
							...
							else if (word_counter >= 4) {     //check whether this looks like a FEM ID word or not:
								if ( (data[index]&0xFFFF) == 0xFFFE) {
									trailer_found = true;
									reading_data = false;
								}
								
								else if ( (data[index] & 0xFFF) == 0) { // found a FEM address
									//printf("FEM ID =%i\n", (data[index]&0xF000)>>12);
									fem_id = (data[index]&0xF000)>>12;
							        reading_data = true;
								}
								/*
								else if (reading_data && trailer_found) {
								*/

								else{
									//printf("Word_counter = %i, Data = %#x", word_counter, data[index]);
									/////////WRITE DATA OUT////////
									//
									//Put data hit in lower 16 bits, bco and amplitude in upper 16 bits:
									//
									data1[cnt] = ( ((ampl & 0x7F) << 24) | ((bco & 0x7F) << 16) | ((data[index]) & 0xFFFF) ); //data[index];

									int32 word = ( ((ampl & 0x7F) << 24) | ((bco & 0x7F) << 16) | ((data[index]) & 0xFFFF) );

									//printf("data = %#x\n", ((data1[cnt]) & 0xFFFF));
									//printf("word to data file = %#x, data = %#x, bco = %#x, ampl = %#x\n", word, data[index], bco, ampl);
									...
									if ( !printData ){
										...
										adc     = (word & 0x07) ;
										chan_id = ((data[index] & 0x200) >>3) | ((data[index] & 0xFC00)>>10); //(word>>9) & 0x3F ;
										chipid  =  (word>>3)  & 0x3F;
										if (cnt%5000 == 0) {
											printf("data %#x, chip_id %2i chan %3i, adc %i, ampl %i\n",word, chipid, chan_id, adc, ampl);
										}
									}
								} // check whether this is data or a FEM ID

						} // check on word_counter

						word_counter++;  // count how many words into the packet we are
