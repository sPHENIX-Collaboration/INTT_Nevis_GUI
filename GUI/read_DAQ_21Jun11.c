...
void
AcquireData(std::string ports, std::string pfi, double sampleHz, FILE* fp, FILE* fp2)
{
#ifdef __linux__
#else
					...
					if ((data[index]&0xFFFFFFFF)!=0 ) 
					{
						...
						if (word_cnt > 5){
							...
							for (int iword = 0; iword < word_cnt-5; iword++){
								...
								//Check whether this is a data word or a detector ID:
								if ( !printData ){
									...
									if (datacnt%500 == 0) {
										...
										printf("data %#x, chip_id %2i chan %3i, adc %i\n",word, chipid, chan_id, adc);
									}
								}
...
main(int argc, char **argv)
{
	...
	try 
	{
		...
		{
			data_threads[i] = new boost::thread(AcquireData,ports[i],pfi[i],sampleHz,f1,f2);
