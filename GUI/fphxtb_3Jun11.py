...
TESTBENCH_FEMLVL1DELAY      = 0X45 # Send the FEM lvl-1 delay command (upper 4 bit means FEM command)
TESTBENCH_BCOSTART    = 0x62 # Send BCO start command to the FEM_IB and the FEM
TESTBENCH_EEPROMWRITE = 0x46 # Send EEPROM write command to the FEM
TESTBENCH_EEPROMREAD  = 0x47 # Send EEPROM read command to the FEM
TESTBENCH_FPGARESET   = 0xC4 # Reset for both FEM and ROC = 0x`C4.  For just ROC = 0x04.  For just FEM = 0x44.
                             # For FEM, ROC and FEM_IB = 0xE4
...
def write_bytes_to_usb(rawbuf,target=None) :
...
def write_bytes_to_target(rawbuf,target=None):
    if target is None:
        #print "write_bytes_to_target: target is None, defaulting to USB"
        return write_bytes_to_usb(rawbuf,target)
...
def create_packet_fphx(word,wedgeaddr=None,femaddr=None):
    ...
    offset += 2
    struct.pack_into("B",  buf,offset,TESTBENCH_FPHX) #
    offset += 1
    if wedgeaddr:
...
def create_packet(dest,data,wedgeaddr=None,femaddr=None):
    ...
    buflen = 4 # Packet markers + length word
...
def write_fphx_cmd(chipid,regid,cmd,data,wedgeaddr=None,femaddr=None,target=None):
    word = make_fphx_cmd(chipid,regid,cmd,data)
    buf = create_packet_fphx(word,wedgeaddr)
    return write_bytes_to_target(buf.raw,target)
...
def write_enable_ro(chipid,initialval,wedgeaddr=None,femaddr=None,target=None):
...
# supply an initial value (this allows the other bits in the register
# to be preserved, for instance).
def write_disable_ro(chipid,initialval,wedgeaddr=None,femaddr=None,target=None):
    chipid = 0x1F & int(chipid)
    regid = 2
    val = initialval
...
def write_pulse_amp(amp,wedgeaddr=None,femaddr=None,target=None):
...
def write_pulse_config(num, deltabco, wedgeaddr=None, femaddr=None, target=None):
...
def write_pulse(wedgeaddr=None,femaddr=None,target=None):
    cmd = TESTBENCH_PULSE
...
def write_pulse_train(num, deltabco, amp, wedgeaddr=None, femaddr=None, target=None):
    ...
    write_pulse_config(num,deltabco,wedgeaddr,femaddr,target)
    return

def write_latch(wedgeaddr=None,femaddr=None,target=None):
...
def write_fo_sync(wedgeaddr=None,femaddr=None,target=None):
...
def write_fem_lvl1_delay(delay,wedgeaddr=None,femaddr=None,target=None):
...
def write_bco_start(wedgeaddr=None,femaddr=None,target=None):
...
def write_eeprom_write(wedgeaddr=None,femaddr=None,target=None):
    cmd = TESTBENCH_EEPROMWRITE
    data = 0x01 # Just need a signal that is high for at least one clock
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
    buf = create_packet(cmd,databuf.raw,wedgeaddr,femaddr)
    if FPHXTB_VERBOSE>0: print 'write_eeprom_write: Send FEM packet = %s' % hexify_bytes(buf)
    write_bytes_to_target(buf.raw,target)
    return

def write_eeprom_read(wedgeaddr=None,femaddr=None,target=None):
    cmd = TESTBENCH_EEPROMREAD
    data = 0x00 # Data should be the EEPROM page address that you want to read.
    data2 = 0x00
    data3 = 0x01
    databuf = create_string_buffer(3)
    struct.pack_into("3B",databuf,0,data, data2, data3)
    buf = create_packet(cmd,databuf.raw,wedgeaddr,femaddr)
    print 'write_eeprom_read: Send FEM packet = %s' % hexify_bytes(buf)
    write_bytes_to_target(buf.raw,target)
    return

# Unmask a single channel for a given chipid
def unmask_chan_fphx(chipid,chan,wedgeaddr=None,femaddr=None,target=None):
    reg = 1
    cmd = FPHX_RESET
    data = 0xFF & chan
...
def unmask_all_fphx(chipid,target=None,wedgeaddr=None,femaddr=None):
    # Just call the channel unmask with global bit
    unmask_chan_fphx(chipid,128,wedgeaddr,femaddr,target)

# Mask a single channel for a given chip id
def mask_chan_fphx(chipid,chan,wedgeaddr=None,femaddr=None,target=None):
    reg = 1
    cmd = FPHX_SET
    data = 0xFF & chan
    word = make_fphx_cmd(chipid,reg,cmd,data)
...
def mask_all_fphx(chipid,wedgeaddr=None,femaddr=None,target=None):
    mask_chan_fphx(chipid,128,wedgeaddr,femaddr,target)
...
# information).
def reset_fphx(wedgeaddr=None,femaddr=None,target=None):
    cmd = TESTBENCH_RESET
    data = 0xFF # data? I just replicate what's in the file 'reset.dat'
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
...
def prog_reset_fphx(chipid,cmd,wedgeaddr=None,femaddr=None,target=None):
    regid = 18
    data = 0
    word = make_fphx_cmd(chipid,regid,cmd,data)
    if FPHXTB_VERBOSE>0: print 'FPHX command = 0x%x' % word
...
def clear_hits_fphx(chipid,wedgeaddr=None,femaddr=None,target=None):
    prog_reset_fphx(chipid,FPHX_SET,wedgeaddr,femaddr,target)
    return

def clear_bco_fphx(chipid,wedgeaddr=None,femaddr=None,target=None):
    prog_reset_fphx(chipid,FPHX_RESET,wedgeaddr,femaddr,target)
    return

# Create and send the calib packet.  Optionally prepend a BCO reset
# the packet payload
def calib_fphx(wedgeaddr=None,femaddr=None,target=None):
    cmd = TESTBENCH_CALIB
    data = 0x01 # data? I just replicate what's in the file 'calib.dat'
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
    if wedgeaddr: wedgeaddr = 0xFF # override user: Write to all wedgeaddr for now.
    buf = create_packet(cmd,databuf.raw,wedgeaddr,femaddr)
