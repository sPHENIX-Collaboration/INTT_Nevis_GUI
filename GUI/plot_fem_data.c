...
void plot_fem_data(char* fname = "C:/FVTX/data_0", int maxbuf = 0, int n_meas = 64, float maxscale = 200.)
{
	gStyle->SetPalette(1);
	gStyle->SetOptFit(1);
	gStyle->SetOptStat(0);
	...
	for (int index=0; index<size/sizeof(unsigned int); bufcnt++)
	{
		...
		else
		{
		  ...
		  chip_id = (data[index]>>3)  & 0x3F;
		  fpga_id = 0;
		  ampl = (data[index]>>24) & 0x7F;
		  bco = (data[index]>>16) & 0x7F;
		  chan_id = ((data[index] & 0x200) >>3) | ((data[index] & 0xFC00)>>10); //data[index]>>9) & 0x7F; //
		  adc = (data[index] & 0x07);
		  ...
		  if (chip_id>0 && chip_id<9){
			  //if (mchip < 1 || mchip > 8 || module < 0 || module > 3) cout << "mchip = " << mchip << ", module = " << module << endl;
			  h1[(int)((module)*NCHIPS + chip_id-1)]->Fill(chan_id,ampl);
		  }
