...
TESTBENCH_FPHX        = 0x83 # FPHX op (Send packet payload to FPHX)
...
TESTBENCH_RESET       = 0x8b # FFR (firefighter reset) for FPHX chip
TESTBENCH_CALIB       = 0x85 # Start calibration sequence
TESTBENCH_PULSEAMP    = 0x86 # Set the pulser amplitude
TESTBENCH_PULSE       = 0x87 # Generate a pulse
TESTBENCH_PULSER      = 0x88 # Configure the pulse train
TESTBENCH_LATCHFPGA   = 0x89 # Send the latch command to the FPGA(s)
TESTBENCH_PULSE_MODULE = 0x8a # Send the module that should be pulsed (bits 0-2 - side, bits 4-7 = wedge location)
TESTBENCH_EEPROM_READ_WRITE     = 0x50 # Read or Write from the EEPROM on the FEM
TESTBENCH_EEPROM_BATCH_DOWNLOAD = 0xd1 # Batch download of EEPROM from FEM to ROC (change from 7 to 11 8/2/11)
...
TESTBENCH_FPGARESET   = 0xE4 # Reset for FEM, FEMIB + ROC = 0xE4, both FEM and ROC = 0x`C4.  For just ROC = 0x84.  For just FEM = 0x44
TESTBENCH_CHECKGLINK  = 0x56 # Check the status of the GLINK lock signal in the FEM IB
TESTBENCH_JTAGSYNC    = 0x54 # Send the JTAG fiber opticy synch command
TESTBENCH_CHECKFEMADDR  = 0x46 # Check the FEM with FEM_ADDR is present and responding
TESTBENCH_USENI  = 0x43 # Set data acquisition for National Instruments rather than DCM (default)
TESTBENCH_SELFTRIG = 0x48 # Set the FEM to self-trigger on input data lines
...
def write_bytes_to_ftp(rawbuf,target):
    ...
    try:
        ...
        session.retrbinary('RETR Status.txt',open('Status.txt','wb').write)
        b = open("Status.txt","rb").read()
#       val = 1
...
def write_pulse_module(module,wedge,wedgeaddr=None,femaddr=None,target=None):
    cmd = TESTBENCH_PULSE_MODULE
    #data = module
    data = ((0xF & wedge) << 4) | (0xF & module)
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
    # For future 16-bit version...
    #data = 0xFFFF & amp
...
def write_check_glink(wedgeaddr=None,femaddr=None,target=None,baud=None):
    cmd = TESTBENCH_CHECKGLINK
    data = 0xff # 
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
    buf = create_packet(cmd,databuf.raw,wedgeaddr,femaddr)
    #if FPHXTB_VERBOSE>0: print 'write_check_glink: Send FPHX packet = %s' % hexify_bytes(buf)
    print 'write_check_glink: Send FPHX packet = %s' % hexify_bytes(buf)
    write_bytes_to_target(buf.raw,target,baud)
    return

def write_check_femaddr(wedgeaddr=None,femaddr=None,target=None,baud=None):
    cmd = TESTBENCH_CHECKFEMADDR
    data = 0xFF # 
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
    buf = create_packet(cmd,databuf.raw,wedgeaddr,femaddr)
    #if FPHXTB_VERBOSE>0: print 'write_check_femaddr: Send FPHX packet = %s' % hexify_bytes(buf)
    print 'write_check_femaddr: Send FPHX packet = %s' % hexify_bytes(buf)
    write_bytes_to_target(buf.raw,target,baud)
    return

def write_self_trig(wedgeaddr=None,femaddr=None,target=None,baud=None):
    cmd = TESTBENCH_SELFTRIG
    data = 0xFF # 
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
    buf = create_packet(cmd,databuf.raw,wedgeaddr,femaddr)
    #if FPHXTB_VERBOSE>0: print 'write_check_femaddr: Send FPHX packet = %s' % hexify_bytes(buf)
    print 'write_self_trig: Send FPHX packet = %s' % hexify_bytes(buf)
    write_bytes_to_target(buf.raw,target,baud)
    return

def write_use_ni(wedgeaddr=None,femaddr=None,target=None,baud=None):
    cmd = TESTBENCH_USENI
    data = 0xFF # 
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
...
def write_jtag_sync(wedgeaddr=None,femaddr=None,target=None,baud=None):
    cmd = TESTBENCH_JTAGSYNC
    data = 0x0F # Just need a signal that is high for at least one clock
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
    buf = create_packet(cmd,databuf.raw,wedgeaddr,femaddr)
    if FPHXTB_VERBOSE>0: print 'write_jtag_sync: Send FEM packet = %s' % hexify_bytes(buf)
    write_bytes_to_target(buf.raw,target,baud)
    return
...
def write_bco_start(wedgeaddr=None,femaddr=None,target=None,baud=None):
    ...
    data = 0x03 # Just need a signal that is high for at least two clocks
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
    buf = create_packet(cmd,databuf.raw,wedgeaddr,femaddr)
...
def calib_fphx(wedgeaddr=None,femaddr=None,target=None,baud=None):
    cmd = TESTBENCH_CALIB
    data = 0x03 # data? I just replicate what's in the file 'calib.dat'
    databuf = create_string_buffer(1)
    struct.pack_into("B",databuf,0,data)
